...
@section('content')
    ...
    <div class="container" style="margin: 20px">
        <div width="100%">
            <img src="{{ 'data:image/jpeg;base64,' . base64_encode($setting['logo'] )}}" alt="" width="70px" style="top:20px;left: 25px;position:absolute">
            <div style="text-align: center; margin-left:60px;">
                <h5 class="m-0">{{ $setting['nama_instansi'] }}</h5>
                <p class="m-0">{{ $setting['alamat_instansi'] }}</p>
                <p class="m-0">Telp. {{ $setting['kontak'] }}, Email : {{ $setting['email'] }} </p>
            </div>
            <hr>
        </div>
        <div class="no_surat" style="text-align: center">
            <h6 class="mb-0"><u>SURAT RUJUK KELUAR</u></h6>
            <p>No. {{ $data['no_rujuk'] }}</p>
        </div>
        <div class="mb-2">
            <p class="mb-0">Kepada Yth. <br/> {{$data['rujuk_ke']}}</p>
            <p class="mb-0">di Tempat</p>
        </div>
        <p class="mb-1" style="text-align: justify">Bersama ini kami beritahukan bahwa kami telah melakukan perawatan/pemeriksaan pasien berikut ini. Mohon pemeriksaan dan penanganan lebih lanjut :</p>
